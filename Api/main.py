...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3001", "http://localhost:3000"],  # Set the list of allowed origins or use "*" for all origins
...
# def validate_answer(questionDocument: str, questionKey: str, answer: str): 
#     try:
#         collection_leaderboard = Depends(get_collection_leaderboard)
#         answer = answer.lower()
...
def validate_answer(room: Room, answerKey: str, answer: str):
    try:
        answer = answer.lower()
        if (room.questionList[room.currentQuestion-1][answerKey] == answer):
            return True
        else:
            return False
    except Exception as e:
        ...
        while True:
            ...
            if message.get("action") == "ready":
                ...
                if currentRoom.user1Ready == True and currentRoom.user2Ready == True:
                    ...
                    currentHiragana = currentQ["hiragana"]
                    sendingResult = {"action": "start", "question": currentHiragana}
                    # sendingResult = json.dumps(sendingResult)
                ...
                # hardcoded to romaji for now
                if (validate_answer(currentRoom, "romaji", message.get("answer"))):
                    ...
                    if(user1Flag == True):
                        await currentRoom.user1WebSocket.send_json({"action":"incorrect"})
                    elif(user1Flag == False):
                        await currentRoom.user2WebSocket.send_json({"action":"incorrect"})
